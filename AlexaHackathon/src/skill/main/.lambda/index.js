...
const SKILL_STATES = {
    ...
    HELP: '_HELPMODE', // this.emit(':tell', ) The user is asking for help.
};
...
const questionStateHandlers = Alexa.CreateStateHandler(SKILL_STATES.TRIVIA, {
    'AllergyAndFoodIntent': function() {
        USDA.getFood(this.event.request.intent.slots.food.value).then((ingredients) => {
            console.log("get food ingredients are", ingredients.ingredients);
            //Is ingredients a list or a string? If it is a string, turn to list
            console.log(this.attributes['allergies']);
            if (this.attributes['allergies'] === null || this.attributes['allergies'] === undefined){
                console.log("do i get here");
                this.attributes['allergies'] = [];
            }
            console.log("allergies are", this.attributes['allergies']);
            this.attributes['allergies'].push(this.event.request.intent.slots.allergy.value);
            console.log("allergies are", this.attributes['allergies']);
            if (this.attributes['allergies'].length == 0){
                this.emit(':tell', this.t('NOT_ALLERGIC_MESSAGE'));
            }
            console.log("gets here");
            var allergiesInFood = Trigger.whatAmIAllergicTo(ingredients, this.attributes['allergies']);
            console.log("allergies in food are ", allergiesInFood);
            if (allergiesInFood.length > 0){
                ...
                const speechOutput = "You are allergic to " + allergiesInFood; //Just names first thing for now
    ...
    'GiveAllergyIntent': function() {
        if (this.attributes['allergies'] == undefined) {
            console.log(this.event.request.intent.slots.allergy.value);
            this.attributes['allergies'] = [this.event.request.intent.slots.allergy.value];
        }
        var list = this.attributes['allergies'];
        console.log(list);
        this.attributes['allergies'] = list.push(this.event.request.intent.slots.allergy.value);
        this.emit(":tell", this.t('ALLERGY_STORED_MESSAGE') + " " + this.event.request.intent.slots.allergy.value);
    },
